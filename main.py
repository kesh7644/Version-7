...
class end:
  ...
  def __init__(self):
        background_color = 'forestgreen' #Background color of the page
        global fwindow
        fwindow = Tk()
        fwindow.title('Exit Box') #Window title
        fwindow.geometry('600x600') #Window size

        self.final_frame = Frame(fwindow, width=700, height=600,bg=background_color)
        self.final_frame.grid(row=1)

        self.final_heading = Label(fwindow,text='Thank You For Trying Out The Quiz  ',  font=('Tw Cen Mt', 22, 'bold'), bg=background_color) #Code for main heading of the page
        self.final_heading.place(x=15, y=35) #Location of the heading

        self.exit_button = Button(fwindow,text='Exit',width=10,bg='red',font=('Tw Cen Mt', 12, 'bold'),command=self.close_end,) #Code for the exit button
        self.exit_button.place(x=260, y=200) #Location of the heading

        self.restart_label = Label(fwindow, text='Do not hesitate to try again' + str(names),font=('Tw Cen Mt', 12, 'bold'),width=40, bg=background_color) #Code for label to try again
        self.restart_label.place(x=110, y=80) #Location of the label
        
        self.final_score = Label(fwindow, text='Your final score is ' + str(score), font=('Tw Cen Mt', 12, 'bold'), width=40, bg=background_color) #Code for quiz summary
